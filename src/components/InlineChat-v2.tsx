...
export default function InlineChatV2({ slug, businessName, apiBase = 'https://app.sistemasparacontratistas.com' }: Props) {
  ...
  // V2 FIX: scroll inside the chat container only, not the whole page
  useEffect(() => {
  ...
  return (
    <div className="max-w-2xl mx-auto">
      {/* Messages area — scrolls internally, not the page */}
      <div
        ref={messagesContainerRef}
        className={`space-y-4 mb-6 transition-all duration-500 ${started ? 'min-h-[200px] max-h-[400px] overflow-y-auto' : ''}`}
      >
        {!started && (
